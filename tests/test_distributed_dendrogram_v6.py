...
from dendro.distributed_dendrogram_v6 import DistributedDendrogramV6
...
def test_1D_pseudo_parallel(ntasks, res, min_npix, min_delta, min_value, show=False):
    ...
    dendrogram = DistributedDendrogramV6.compute_pseudo_parallel(
    ...
    if show:
        ...
        local_dendrograms = (
            DistributedDendrogramV6().compute_local_dendrogram_pseudo_parallel(
...
@pytest.mark.parametrize("min_value", ["min", 0.2])
def test_1D(mpi_ranks, res, min_npix, min_delta, min_value):
    from dendro.utils import get_1d_data
...
def test_2D(mpi_ranks, res, n_peaks):
    from dendro.utils import get_2d_data
...
def test_example_pseudo_parallel(ntasks, min_value, min_delta, min_npix):
    ...
    d = DistributedDendrogramV6.compute_pseudo_parallel(data, ntasks, **kwargs)
