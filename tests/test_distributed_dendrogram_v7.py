...
from dendro.distributed_dendrogram_v7 import DistributedDendrogramV7, Dendrogram, ht, np
...
@pytest.mark.parametrize("min_value", ["min"])
@pytest.mark.parametrize("random", [True, False])
def test_1D(mpi_ranks, res, min_npix, min_delta, min_value, random, show=False):
    from dendro.utils import get_1d_data, compare_dendrograms
    ...
    dendrogram = DistributedDendrogramV7.compute(data=data, **kwargs)
    reference_dendrogram = Dendrogram.compute(data=data.numpy(), **kwargs)

    all_index_maps = ht.comm.allgather(dendrogram.index_map)
    for i in range(1, ht.comm.size):
...
@pytest.mark.parametrize("n_peaks", [2, 3, 4])
...
def test_2D(ntasks, res, n_peaks, min_npix, min_value, min_delta, show=False):
    ...
    reference_dendrogram = Dendrogram.compute(data.numpy(), **kwargs)
    dendrogram = DistributedDendrogramV7.compute(data, **kwargs)
...
@pytest.mark.parametrize("min_delta", [1])
@pytest.mark.parametrize("min_npix", [10])
def test_example(mpi_ranks, min_value, min_delta, min_npix, show=False):
    from astropy.io.fits import getdata
    import astrodendro
    import numpy as np
    from dendro.utils import compare_dendrograms
    ...
    d_ref = astrodendro.Dendrogram.compute(data, **kwargs)
    d = DistributedDendrogramV7.compute(data, **kwargs)
    if show:
        import matplotlib.pyplot as plt
        from dendro.utils import plot_astrodendro_tree_2D
