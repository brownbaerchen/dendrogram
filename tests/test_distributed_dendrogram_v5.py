...
def test_1D_v5_pseudo_parallel(ntasks, res, min_npix, min_delta, min_value, show=False):
    ...
    halo_size = 4
    DistributedDendrogramV5.halo_size = halo_size
    ...
    if show:
        ...
        for i, d in enumerate(local_dendrograms):
            plot_astrodendro_leaves(axs_top[2 * i], x.numpy(), data.numpy(), d.trunk)

        halo_dendrograms = (
            DistributedDendrogramV5().get_halo_dendrograms_pseudo_parallel(
                ntasks=ntasks, halo_size=halo_size, **kwargs
            )
        )
        for i, d in enumerate(halo_dendrograms):
            plot_astrodendro_leaves(
                axs_top[2 * i + 1], x.numpy(), data.numpy(), d.trunk
            )
...
if __name__ == "__main__":
    ...
    test_1D_v5_pseudo_parallel(2, 32, 0, 0.0, 0.0, show=True)
