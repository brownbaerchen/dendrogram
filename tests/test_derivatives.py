import pytest


def test_derivative_2D():
    import heat as ht
    from dendro.derivative import compute_derivative

    x = ht.linspace(0, 1, 64)
    y = ht.linspace(0, 1, 64)

    X, Y = ht.meshgrid(x, y, indexing="ij")

    data = X
    expect_derivative = ht.ones_like(data)
    derivative = compute_derivative([X, Y], data, axis=0)
    assert ht.allclose(expect_derivative, derivative)

    data = Y
    expect_derivative = ht.ones_like(data)
    derivative = compute_derivative([X, Y], data, axis=1)
    assert ht.allclose(expect_derivative, derivative)

    data = X + Y
    expect_derivative = ht.ones_like(data)
    derivative = compute_derivative([X, Y], data, axis=1)
    assert ht.allclose(expect_derivative, derivative)

    data = X * Y
    expect_derivative = X.copy()
    derivative = compute_derivative([X, Y], data, axis=1)
    assert ht.allclose(expect_derivative, derivative)

    data = X * Y
    expect_derivative = Y.copy()
    derivative = compute_derivative([X, Y], data, axis=0)
    assert ht.allclose(expect_derivative, derivative)

    data = X**2
    expect_derivative = 2 * X
    derivative = compute_derivative([X, Y], data, axis=0)
    assert ht.allclose(expect_derivative[1:-1, :], derivative[1:-1, :])

    data = Y**2 * X**2
    expect_derivative = 2 * Y * X**2
    derivative = compute_derivative([X, Y], data, axis=1)
    assert ht.allclose(expect_derivative[:, 1:-1], derivative[:, 1:-1])


@pytest.mark.parametrize("f", [1, 2, 4])
def test_find_extrema(f):
    import heat as ht
    from dendro.derivative import find_extrema, compute_derivative

    x = ht.linspace(0, 2 * ht.pi, 256)
    data = ht.sin(f * x) ** 2
    deriv = compute_derivative([x], data)

    extrema = find_extrema(deriv)
    tols = {1: 0.9, 2: 1.8, 4: 3.8}
    expect_extrema = ((f * x) % (ht.pi / 2)) <= tols[f] * x[1]
    assert ht.allclose(expect_extrema, extrema)


@pytest.mark.parametrize("f", [1, 2, 4])
def test_find_minima(f):
    import heat as ht
    from dendro.derivative import find_minima, compute_derivative

    x = ht.linspace(0, 2 * ht.pi, 256)
    data = ht.sin(f * x) ** 2
    deriv = compute_derivative([x], data)

    minima = find_minima([x], data)
    tols = {1: 0.9, 2: 1.6, 4: 3.6}
    expect_extrema = ((f * x) % (ht.pi)) <= tols[f] * x[1]
    assert ht.allclose(expect_extrema, minima)


@pytest.mark.parametrize("f", [1, 2])
def test_find_minima_2D(f):
    import heat as ht
    from dendro.derivative import find_minima, compute_derivative

    x = ht.linspace(0, 2 * ht.pi, 32)
    y = ht.linspace(0, 2 * ht.pi, 32)
    X, Y = ht.meshgrid(x, y, indexing="ij")
    data = ht.sin(f * X) ** 2 + ht.sin(f * Y) ** 2
    deriv = compute_derivative([X, Y], data)

    minima = {i: find_minima([X, Y], data, axis=i) for i in range(data.ndim)}
    tols = {1: 0.9, 2: 1.6}
    expect_extrema = {}
    expect_extrema[0] = ((f * X) % (ht.pi)) <= tols[f] * x[1]
    expect_extrema[1] = ((f * Y) % (ht.pi)) <= tols[f] * x[1]

    for i in minima.keys():
        assert ht.allclose(expect_extrema[i], minima[i])


if __name__ == "__main__":
    test_find_minima_2D(1)
