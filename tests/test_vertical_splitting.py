...
def test_vertical_distribution_serial(n, ndim, ntasks, halo_size):
    ...
    min_vals = [np.min(local_data[i][np.isfinite(local_data[i])]) for i in range(ntasks)]
    max_vals = [np.max(local_data[i][np.isfinite(local_data[i])]) for i in range(ntasks)]
