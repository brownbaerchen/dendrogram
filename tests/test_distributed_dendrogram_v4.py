...
def test_1D_v4_pseudo_parallel(ntasks, res):
    from dendro.utils import get_1d_data

    x, data = get_1d_data(res)

    dendrogram = DistributedDendrogramV4.compute_pseudo_parallel(data.numpy(), ntasks)
    reference_dendrogram = Dendrogram.compute(data.numpy())

    # import matplotlib.pyplot as plt
    # fig, axs = plt.subplots(2, ntasks)
    # local_dendrograms = DistributedDendrogramV4.compute_local_dendrogram_pseudo_parallel(data.numpy(), ntasks)
...
def test_2D_v4_pseudo_parallel(ntasks, res, n_peaks):
...
def test_2D_v4(mpi_ranks, res, n_peaks):
    from dendro.utils import get_2d_data

    _, _, data = get_2d_data(res, n_peaks)

    dendrogram = DistributedDendrogramV4.compute(data)
    reference_dendrogram = Dendrogram.compute(data.numpy())
    compare_dendrograms(reference_dendrogram, dendrogram)
...
if __name__ == "__main__":
    ...
    test_1D_v4_pseudo_parallel(2, 128)
    # test_2D_v4_pseudo_parallel(2, 32, 2)
